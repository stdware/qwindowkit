function(run_step name)
    execute_process(COMMAND ${ARGN} RESULT_VARIABLE result
        OUTPUT_VARIABLE output ERROR_VARIABLE error TIMEOUT 90)
...
run_step(configure "${CMAKE_COMMAND}" -S "${SOURCE_DIR}" -B "${WORK_DIR}/build"
    ${generator_args} "-DCMAKE_PREFIX_PATH=${prefix}" "-DQT_DIR=${QT_DIR}" "-DQt6_DIR=${QT_DIR}"
    "-DCMAKE_BUILD_TYPE=${BUILD_CONFIG}")
run_step(build "${CMAKE_COMMAND}" --build "${WORK_DIR}/build" --config "${BUILD_CONFIG}")
...
run_step(runtime "${consumer}" "${SOURCE_DIR}/Window.qml" "${prefix}/qml")
